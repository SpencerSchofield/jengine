...
namespace Jengine {
	...
	void Camera::setPosition(
			UNIMPLEMENTED glm::vec3 position)
	{
	}

	void Camera::setAngle(
			UNIMPLEMENTED glm::vec3 angle)
	{
	}

	void Camera::setPositionAndAngle(
			glm::mat4 matrix)
	{
		this->position = matrix;
	}

	UNSURE void Camera::modifyPosition(
			UNIMPLEMENTED glm::vec3 position)
	{
		this->position = glm::translate(this->position, position);
	}

	void Camera::modifyAngle(
			UNIMPLEMENTED glm::vec3 angle)
	{

	}

	UNSURE void Camera::modifyPositionAndAngle(
			glm::mat4 matrix)
	{
		this->position += matrix;
		this->updated = false;
